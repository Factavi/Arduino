...
void loop() {
  ...
  analogWrite(LEDPin, dialValue/4);
  int oldSensorValue = sensorValue;
  sensorValue = analogRead(sensorPin);
  int sensorDifference = sensorValue - oldSensorValue;
  
  if ( (sensorValue < sensorLowThreshold) || (sensorValue > sensorHighThreshold) ) {
    Serial.println("Out of light range.");
    ...
    if ( sensorDifference > dialValue ) { 
      Serial.println("I notice I am nearing a wall");
